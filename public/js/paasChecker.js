var passChecker = (function(){
    ...
    var getNodes = function(){
        ...
        checkerBlock   = document.querySelector('.pass-checker');
    ...
    var publicPassEvent = function(isPublic){
        if (isPublic === true) {
            publicWarnElem.innerHTML = i18n.publicWarn;
            setUnreliableStyle();
        } else {
            publicWarnElem.innerHTML = '';
        }
    };

    var checkPublic = function(pass){
        var request = new XMLHttpRequest();
        request.open('POST', '/?area=ajaxPassChecker', true);
        request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
        request.onreadystatechange = function() {
            if (this.readyState === 4){
                if (this.status >= 200 && this.status < 400){
                    var result = JSON.parse(this.responseText);
                    if (result.foundInPublic === true) {
                        publicPassEvent(true);
                    }
                }
            }
        };
        request.send("pass=" + encodeURI(pass));
        request = null;
        publicPassEvent(false);
    };
...
*/
                ...
                */
                ...
                checkerBlock.style.backgroundColor = color;
            } else {
                resultBlock.style.display = 'none';
                checkerBlock.style.backgroundColor = '#ffffff';
            }
        };
